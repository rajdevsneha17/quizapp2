    ...
    const fetchQuestions = async () => {
      try {
        ...
        const formattedQuestions = response.data.results.map((question) => {
          ...
          const shuffledOptions = options.sort(() => Math.random() - 0.5); // Shuffle options to randomize their order
          return {
            question: question.question,
            options: shuffledOptions.map(option => ({
              option,
              score: option === question.correct_answer ? 4 : 0 // Assign scores, with correct answer getting 4 and others 0
            }))
          };
        });
        setQuestions(formattedQuestions);
